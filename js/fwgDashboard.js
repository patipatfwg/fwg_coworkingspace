...
{
  // const driver = new Driver({ opacity: 0.4, showButtons: true, stageBackground: '#ffffff', allowClose: true });
  var driver;
  var seat_id = "#"+seat;
  $(seat_id).click(function(){
    ...
    request.done(function(msg) {
      ...
      {
        console.log(code);
        driver = new Driver({ opacity: 0.4, showButtons: false, stageBackground: '#ffffff', allowClose: true });
        // driverprop = "{ opacity: 0.4, showButtons: false, stageBackground: '#ffffff', allowClose: true }";
        title +="<br>"+msg.first_name_en+" "+msg.last_name_en;
        description +=msg.booking_employee_start+" - "+msg.booking_employee_end+" <button>ยกเลิกการจอง</button>";
      }
      else if(code==404)
      {
        console.log(code);
        driver = new Driver({ opacity: 0.4, showButtons: true, stageBackground: '#ffffff', allowClose: true });
        // driverprop = "{ opacity: 0.4, showButtons: true, stageBackground: '#ffffff', allowClose: true }";
        title ="Booking: "+seat;
      }
      // driver = new Driver(driverprop);
      driver.highlight({
        element: seat_id,
        popover: {
            title: title,
